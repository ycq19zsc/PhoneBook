...
using ZSC.PhoneBook.Migrations.SeedData;
...
namespace ZSC.PhoneBook.Tests
{
    public abstract class PhoneBookTestBase : AbpIntegratedTestBase<PhoneBookTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected PhoneBookTestBase()
        {
        ...
        protected void UsingDbContext(Action<PhoneBookDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<PhoneBookDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<PhoneBookDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<PhoneBookDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<PhoneBookDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<PhoneBookDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<PhoneBookDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<PhoneBookDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<PhoneBookDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<PhoneBookDbContext, Task<T>> func)
        {
            T result;
